...
async def test_mc_l13_only(dut):
    ...
    md, ld = load_golden('model_c_int8')
    i = 13; m = md[i]; d = ld[i]
    ...
    dut._log.info(f"L13 {m['in_h']}x{m['in_w']}x{m['in_c']} -> {m['out_h']}x{m['out_w']}x{m['out_c']} tile={m.get('tile_h',0)}x{m.get('tile_w',0)}")
    ...
    owned_words = owned[::4] | owned[1::4] | owned[2::4] | owned[3::4]  # word is owned if either element is owned
    # Only compare owned words
    ref_owned = ref[owned_words[:nw]]
    got_owned = got[owned_words[:nw]]
    mm = np.where(got_owned != ref_owned)[0]
    if len(mm) == 0:
        dut._log.info(f"PASS L13: {len(ref_owned)}/{len(ref_owned)} (owned channels only)")
    else:
        # Also check that non-owned words are 0 in golden (they should be)
        dut._log.error(f"FAIL L13: {len(mm)}/{len(ref_owned)} owned mismatches, first at w[{mm[0]}] exp={ref_owned[mm[0]]:08X} got={got_owned[mm[0]]:08X}")
