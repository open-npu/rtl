...
@cocotb.test()
async def test_add_layers(dut):
    cocotb.start_soon(Clock(dut.clk, 10, unit="ns").start())
    await reset(dut)
    wb = WbSlave(dut, dut.clk); mem = WbMasterMem(dut, dut.clk)
    cocotb.start_soon(mem.run())
    md, ld = load_golden('model_b_int16')
    # Test all Add layers: L5, L8, L12, L15, L19, L22
    add_layers = [i for i, m in enumerate(md) if m['op_type'] == 4]
    dut._log.info(f"Add layers: {add_layers}")
    for i in add_layers:
        m = md[i]; d = ld[i]
        # Populate weights, params, input, input_b
        ...
        dut._log.info(f"L{i} Add {m['in_h']}x{m['in_w']}x{m['in_c']} -> {m['out_h']}x{m['out_w']}x{m['out_c']} tile={m.get('tile_h',0)}x{m.get('tile_w',0)}")
