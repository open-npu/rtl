...
@cocotb.test()
async def test_conv_7x7(dut):
    cocotb.start_soon(Clock(dut.clk, 10, unit="ns").start())
    await reset(dut)
    wb = WbSlave(dut, dut.clk); mem = WbMasterMem(dut, dut.clk)
    cocotb.start_soon(mem.run())
    md, ld = load_golden('model_b_int16')
    # Test small Conv2D layers: L9, L16, L17, L20, L21 (7x7 tiles)
    for i in [9, 16, 17, 20, 21]:
        m = md[i]; d = ld[i]
        ...
        dut._log.info(f"L{i} Conv2D {m['in_h']}x{m['in_w']}x{m['in_c']} -> {m['out_h']}x{m['out_w']}x{m['out_c']} tile={m.get('tile_h',0)}x{m.get('tile_w',0)}")
