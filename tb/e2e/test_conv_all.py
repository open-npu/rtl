...
@cocotb.test()
async def test_conv_all(dut):
    cocotb.start_soon(Clock(dut.clk, 10, unit="ns").start())
    await reset(dut)
    wb = WbSlave(dut, dut.clk); mem = WbMasterMem(dut, dut.clk)
    cocotb.start_soon(mem.run())
    md, ld = load_golden('model_b_int16')
    # Test all Conv2D, Pool, FC layers (skip Add which we already verified)
    conv_layers = [i for i, m in enumerate(md) if m['op_type'] in [0, 2, 3]]
    dut._log.info(f"Testing layers: {conv_layers}")
