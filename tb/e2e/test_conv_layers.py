...
@cocotb.test()
async def test_conv_layers(dut):
    cocotb.start_soon(Clock(dut.clk, 10, unit="ns").start())
    await reset(dut)
    wb = WbSlave(dut, dut.clk); mem = WbMasterMem(dut, dut.clk)
    cocotb.start_soon(mem.run())
    md, ld = load_golden('model_b_int16')
    test_layers = [0, 3, 4, 9]  # L0:7x7, L3:3x3, L4:1x1, L9:3x3 stride2
    for i in test_layers:
        mem.populate(md[i]['ddr_wgt_addr'], ld[i]['wgt'])
        mem.populate(md[i]['ddr_param_addr'], ld[i]['param'])
    for i in test_layers:
        m=md[i]; d=ld[i]
        ...
        dut._log.info(f"L{i} {m['in_h']}x{m['in_w']}x{m['in_c']} -> {m['out_h']}x{m['out_w']}x{m['out_c']} k={m['kernel_h']}x{m['kernel_w']} tile={m.get('tile_h',0)}x{m.get('tile_w',0)}")
