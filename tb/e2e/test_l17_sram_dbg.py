import os, sys, cocotb, numpy as np
from cocotb.clock import Clock
sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
from test_npu_dma_e2e import WbSlave, WbMasterMem, reset, load_golden, program_layer, run_layer_and_wait

@cocotb.test()
async def test_l17_sram_dbg(dut):
    cocotb.start_soon(Clock(dut.clk, 10, unit="ns").start())
    await reset(dut)
    wb = WbSlave(dut, dut.clk); mem = WbMasterMem(dut, dut.clk)
    cocotb.start_soon(mem.run())
    md, ld = load_golden('model_b_int16')
    i = 17; m = md[i]; d = ld[i]
    # Populate weight in DDR
    mem.populate(m['ddr_wgt_addr'], d['wgt'])
    dut._log.info(f"L17 wgt DDR=0x{m['ddr_wgt_addr']:08X} size={m['dma_wgt_size']}")
    # Program layer and start
    if len(d['param']) > 0: mem.populate(m['ddr_param_addr'], d['param'])
    mem.populate(m['ddr_in_addr'], d['input'])
    await program_layer(wb, m)
    # Run just enough to load weights, then check
    done = await run_layer_and_wait(wb, dut, timeout=600000000)
    # Check SRAM weight directly via Wishbone (if SRAM is memory-mapped)
    # Actually check DDR-to-SRAM mapping by reading output
    nw=m['n_output_words']; ref=d['output']; oa=m['ddr_out_addr']
    got=np.array([mem.mem.get(oa+j*4,0) for j in range(nw)],dtype=np.uint32)
    mm=np.where(got!=ref)[0] if len(ref)==nw else np.arange(nw)
    if len(ref)!=nw: dut._log.error(f"FAIL L17: size {len(ref)} vs {nw}")
    elif len(mm)==0: dut._log.info(f"PASS L17: {nw}/{nw}")
    else: dut._log.error(f"FAIL L17: {len(mm)}/{nw} first w[{mm[0]}] exp={ref[mm[0]]:08X} got={got[mm[0]]:08X}")
