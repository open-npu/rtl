...
@cocotb.test()
async def test_ma_l38_only(dut):
    ...
    dut._log.info(f"L38 {m['in_h']}x{m['in_w']}x{m['in_c']} -> {m['out_h']}x{m['out_w']}x{m['out_c']} k={m['kernel_h']}x{m['kernel_w']} tile={m.get('tile_h',0)}x{m.get('tile_w',0)}")
    ...
    if len(ref)!=nw: dut._log.error(f"FAIL L38: size {len(ref)} vs {nw}")
    elif len(mm)==0: dut._log.info(f"PASS L38: {nw}/{nw}")
    else: dut._log.error(f"FAIL L38: {len(mm)}/{nw} first w[{mm[0]}] exp={ref[mm[0]]:08X} got={got[mm[0]]:08X}")
