...
@cocotb.test()
async def test_ma_l62_only(dut):
    ...
    if len(ref)!=nw: dut._log.error(f"FAIL L62: size {len(ref)} vs {nw}")
    elif len(mm)==0: dut._log.info(f"PASS L62: {nw}/{nw}")
    else: dut._log.error(f"FAIL L62: {len(mm)}/{nw} first w[{mm[0]}] exp={ref[mm[0]]:08X} got={got[mm[0]]:08X}")
