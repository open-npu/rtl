"""
MobileNetV2-Tiny End-to-End RTL Test.

Runs 10 layers of MobileNetV2-Tiny through the RTL compute unit,
comparing output against Python golden reference.

Prerequisites: run gen_mobilenet_golden.py first to generate golden data.
...
def set_cfg(dut, cfg, array_size, act_base=0, out_base=0):
    ...
    op_type = 0 if cfg['op_type'] == 'conv2d' else 1  # 0=conv2d, 1=dw_conv
    ...
    dut.ppu_bias_en.value = 1
    dut.ppu_zp_en.value = 1
    # ReLU enabled for layers with relu6 flag
...
async def run_layer(dut, layer_idx, cfg, wgt_words, param_words,
    ...
    # Calculate SRAM regions: input at offset 0, output at offset past input
    # to avoid overlap
    act_base = 0  # input starts at word 0
    out_base = len(input_words)  # output starts after input
    ...
    # Wait for done — large layers can take 100k+ cycles
    timeout = 500000
    done_seen = False
    final_cyc = 0
    first_wr_addr = None
    for cyc in range(timeout):
        ...
        await ReadOnly()
        try:
            # Capture first write address for debug
    ...
    for i in range(n_out_words):
        try:
            ...
            got = 0xDEADBEEF  # X values in SRAM
    ...
    if mismatches > 0:
        ...
        # Decode bytes for readable error
        got_bytes = [(got >> (b*8)) & 0xFF for b in range(4)]
        exp_bytes = [(exp >> (b*8)) & 0xFF for b in range(4)]
        return False, (f"Layer {layer_idx}: {mismatches}/{n_out_words} word mismatches. "
                       f"First at word {wi}: got {got_bytes} exp {exp_bytes}. "
...
@cocotb.test()
async def test_mobilenet_layer0(dut):
    """MobileNetV2-Tiny Layer 0: Conv2D 3×3, stride=2, 3→8, ReLU6."""
...
@cocotb.test()
async def test_mobilenet_layer1(dut):
    """MobileNetV2-Tiny Layer 1: DWConv 3×3, stride=1, 8ch, ReLU6."""
    cocotb.start_soon(Clock(dut.clk, 10, unit="ns").start())
    await reset_dut(dut)
...
@cocotb.test()
async def test_mobilenet_layer2(dut):
    """MobileNetV2-Tiny Layer 2: Conv2D 1×1, 8→4, linear."""
    cocotb.start_soon(Clock(dut.clk, 10, unit="ns").start())
    await reset_dut(dut)
...
@cocotb.test()
async def test_mobilenet_all_layers(dut):
    """MobileNetV2-Tiny: run all 10 layers sequentially."""
    cocotb.start_soon(Clock(dut.clk, 10, unit="ns").start())
    await reset_dut(dut)
    ...
    for layer_idx in range(len(metadata)):
        # Ensure we're not in ReadOnly phase before resetting
        await RisingEdge(dut.clk)
        await reset_dut(dut)
        ...
        if not ok:
            # Continue to see which layers fail
            pass

    # Summary
    passed = sum(1 for _, ok, _ in results if ok)
    total = len(results)
    dut._log.info(f"MobileNetV2-Tiny: {passed}/{total} layers passed")
