"""
MobileNetV2-Tiny INT8 RTL Test — Real Model (wider weight/bias range).

Verifies RTL hardware produces bit-exact results matching the Python reference
on a real quantized INT8 neural network model (MobileNetV2-Tiny, 10 layers).

Compared to the base test_mobilenet_e2e.py, this test uses wider weight ranges
([-64,65] vs [-8,9]) and larger biases ([-1000,1000] vs [-100,100]), providing
additional stress testing of the INT8 datapath with higher accumulator values.

The golden data is generated by gen_model_int8_rtl_golden.py which:
1. Builds the model with INT8 quantized weights (wider range)
2. Computes Python reference golden outputs (bit-exact with RTL PPU)
3. Packs data in SRAM format for RTL consumption

Prerequisites: run gen_model_int8_rtl_golden.py first.
...
async def reset_dut(dut):
    """Apply reset with INT8 mode (cfg_int16=0)."""
    dut.rst_n.value = 0
    dut.start.value = 0
...
def unpack_i8(word, idx):
    """Unpack int8 from uint32 word at byte index (0-3)."""
    val = (word >> (idx * 8)) & 0xFF
    if val >= 0x80:
        val -= 0x100
    return val
...
async def run_layer(dut, layer_idx, cfg, wgt_words, param_words,
    ...
    if mismatches > 0:
        ...
        got_elems = [unpack_i8(got, j) for j in range(4)]
        exp_elems = [unpack_i8(exp, j) for j in range(4)]
        return False, (f"Layer {layer_idx}: {mismatches}/{n_out_words} word mismatches. "
                       f"First at word {wi}: got {got_elems} exp {exp_elems}. "
                       f"out_base={out_base}")

    return True, f"Layer {layer_idx}: PASS ({final_cyc} cycles, bit-exact)"


@cocotb.test()
async def test_model_int8_layer0(dut):
    """INT8 Model Layer 0: Conv2D 3x3, stride=2, 3->8 (Python ref golden)."""
...
@cocotb.test()
async def test_model_int8_all_layers(dut):
    """INT8 Model: all 10 layers vs Python ref golden (bit-exact)."""
    cocotb.start_soon(Clock(dut.clk, 10, unit="ns").start())
    await reset_dut(dut)
    ...
    total = len(results)
    dut._log.info(f"INT8 Model RTL: {passed}/{total} layers bit-exact vs Python ref")
