...
# Not collected: skip=True would fail the Phase-1 SKIP gate.
async def _retired_pipeline_conv(dut):
...
# Not collected: skip=True would fail the Phase-1 SKIP gate.
async def _retired_pipeline_dwconv(dut):
...
# Not collected: expect_fail / skip would fail the Phase-1 FAIL/SKIP gate.
async def _retired_dma_e2e_tiling_int16_db_en(dut):
...
# Not collected: expect_fail / skip would fail the Phase-1 FAIL/SKIP gate.
async def _retired_dma_e2e_tiling_int8_db_en_fused(dut):
...
async def test_dma_e2e_single_layer_int8_with_stride(dut):
    ...
    # Non-tiled 1D loads advance by bytes-per-word. stride=0 means the DMA
    # default of +4; an explicit 4 must be bit-exact with that default.
    in_stride = 4
    out_stride = 0
...
# Not collected: skip=True would fail the Phase-1 SKIP gate.
async def _retired_dma_e2e_noncontiguous_load_stride(dut):
...
# RETIRED (2026-08-26): premise invalid — 1D stores are always contiguous
# (+4/word); CSR 0x114 only affects 2D-tiled row advance. Same reason as
# test_dma_e2e_noncontiguous_load_stride. Not collected (skip=True would
# fail the Phase-1 SKIP gate).
async def _retired_dma_e2e_noncontiguous_store_stride(dut):
...
@cocotb.test()
async def test_pool_avg_global_8x8(dut):
    """Regression: reciprocal LUT index 64 must not truncate to index 0."""
    cocotb.start_soon(Clock(dut.clk, 10, unit="ns").start())
    await reset(dut)
    ...
    meta, data = gen_pooling_test(
        mode='avg', pool_h=8, pool_w=8, pool_sh=8, pool_sw=8,
        in_h=8, in_w=8, in_c=4, global_pool=True, seed=64,
    )
    mem.populate(POOL_IN_ADDR, data['input_words'])
    mem.populate(POOL_PARAM_ADDR, data['param_words'])
    ...
    assert done, "Global AvgPool 8x8 did not complete"

    mismatches = []
    for i in range(meta['n_output_words']):
        got = mem.mem.get(POOL_OUT_ADDR + i * 4, None)
        exp = int(data['output_words'][i])
        if got != exp:
            mismatches.append((i, exp, got))
    assert not mismatches, f"GlobalPool 8x8 mismatches: {mismatches[:10]}"
    dut._log.info("Global AvgPool 8x8 PASSED bit-exact")
...
# Not collected: skip=True would fail the Phase-1 SKIP gate.
async def _retired_full_model_allops(dut):
...
# RETIRED (2026-08-17): superseded by the 5-model SoC E2E matrix (int8+int16, bit-exact). June golden contract drifted; coverage here is fully redundant.
# Not collected: skip=True would fail the Phase-1 SKIP gate.
async def _retired_full_model_allops_int16(dut):
...
# RETIRED (2026-08-17): superseded by the 5-model SoC E2E matrix (int8+int16, bit-exact). June golden contract drifted; coverage here is fully redundant.
# Not collected: skip=True would fail the Phase-1 SKIP gate.
async def _retired_full_model_allops_128(dut):
