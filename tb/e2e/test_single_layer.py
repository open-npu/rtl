"""Run a single layer standalone test.

Usage: make SIM=verilator DUT=npu_top MODULE=e2e.test_single_layer
Set env var LAYER_IDX to select layer.
"""
...
async def test_single_layer(dut):
    idx = int(os.environ.get('LAYER_IDX', '23'))
    ...
    ops = {0:'Conv2D', 2:'FC', 3:'Pool', 4:'Add'}
    dut._log.info(f"L{idx} {ops.get(m['op_type'],'?')} {m['in_h']}x{m['in_w']}x{m['in_c']} -> {m['out_h']}x{m['out_w']}x{m['out_c']} tile={m.get('tile_h',0)}x{m.get('tile_w',0)}")
    ...
    if len(ref)!=nw: dut._log.error(f"FAIL L{idx}: size {len(ref)} vs {nw}")
    elif len(mm)==0: dut._log.info(f"PASS L{idx}: {nw}/{nw}")
    else: dut._log.error(f"FAIL L{idx}: {len(mm)}/{nw} first w[{mm[0]}] exp={ref[mm[0]]:08X} got={got[mm[0]]:08X}")
