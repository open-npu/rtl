#!/usr/bin/env python3
"""
MobileNetV2-Tiny golden data generator for RTL simulation.

Produces per-layer SRAM preload data (weights, params, input activations)
and expected output activations. The RTL testbench iterates through layers,
loading data into SRAM and verifying output matches.

Weight format: contiguous OHWI byte packing
  For Conv2D: wgt[oc][kh][kw][in_c], packed as col c starts at byte c*k_depth
  For DW Conv: wgt[ch][kh][kw], packed sequentially

Param format: 4 words per channel
  Word 0: M (mult, 15-bit) | S (shift, 6-bit) packed as (S << 15) | M  [lower 21 bits]
  Word 1: bias low 32 bits (signed)
  Word 2: bias high 8 bits (sign-extended)
  Word 3: zero_point (signed int16, lower 16 bits)

Activation format: NHWC, byte-packed
...
def ref_postproc(acc, M_arr, S_arr, bias_arr, zp_arr, relu6=False, clamp=(-128, 127)):
    ...
    for c in range(n_ch):
        ...
        if s > 0:
            ...
            shifted = rounded >> s  # arithmetic shift (Python int >> preserves sign)
        else:
            shifted = product
        # Stage 4: Add zero point
        zp_result = shifted + np.int64(zp_arr[c])
        # Clamp to [-128, 127]
        ch_out = np.clip(zp_result, -128, 127)
...
def pack_params_for_sram(M_arr, S_arr, bias_arr, zp_arr, n_ch):
    ...
    for c in range(n_ch):
        ...
        bias = int(bias_arr[c])
        # Convert bias to unsigned 64-bit representation
        if bias < 0:
            bias_u64 = bias + (1 << 64)
        else:
            bias_u64 = bias
...
def pack_activations_for_sram(act_nhwc):
    """Pack NHWC activation tensor as SRAM words.
...
def build_mobilenetv2_tiny():
    ...
    Returns:
        ...
        layer_data: list of dicts with keys:
            'wgt_words': SRAM weight words
            'param_words': SRAM param words
            'input_words': SRAM input activation words
            'output_words': SRAM expected output words
            'output_nhwc': numpy output for chaining
    """
    np.random.seed(2024)
    ...
    # Generate random input: 16×16×3
    input_nhwc = np.random.randint(-64, 64, (16, 16, 3), dtype=np.int8)
    ...
    def add_dw(in_act, in_h, in_w, ch, stride=1, relu6=True):
        ...
        # Pack for SRAM
        wgt_words = pack_dw_weights_for_sram(w, ch, 9)
    ...
    # Skip GlobalAvgPool + FC — not supported in compute unit directly
    # (would need controller orchestration or separate test)
...
def print_summary(layers):
    ...
    print(f"MobileNetV2-Tiny RTL Golden: {len(layers)} layers")
    for i, cfg in enumerate(layers):
        kh, kw = cfg['kernel_h'], cfg['kernel_w']
        print(f"  Layer {i:2d}: {cfg['op_type']:8s} "
              f"[{cfg['in_h']}×{cfg['in_w']}×{cfg['in_c']}] → "
              f"[{cfg['out_h']}×{cfg['out_w']}×{cfg['out_c']}] "
              f"k={kh}×{kw} s={cfg['stride_h']} "
              f"{'ReLU6' if cfg['relu6'] else 'linear'}")


def save_golden(output_dir):
    """Generate and save golden data to files for cocotb test."""
    layers, layer_data, input_nhwc = build_mobilenetv2_tiny()
